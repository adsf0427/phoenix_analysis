...
def main():
    ...
    batch = 0
    model.train()
    for obs, label, mask in data_loader:
        train_loss = []
        train_accs = []
        mini_loss = []
        mini_accs = []

        obs = obs[0].to(dtype=torch.float32, device=device)
        labels = label.to(dtype=torch.float32, device=device)
        masks = mask.to(dtype=torch.bool, device=device)
        logits = model(obs)
        logits = logits.masked_fill_(masks, -torch.inf)
        ...
        if batch % 10 == 0:
            torch.save(model.state_dict(), 'chong.pth')
