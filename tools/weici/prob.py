...
net = nn.Sequential(b1, nn.Conv1d(256, 1, kernel_size=1, stride=1), nn.Flatten(), nn.Linear(34, 4))
model = net
model.load_state_dict(torch.load('weici.pth', map_location=device))
...
def update(line):
    ...
    return logits.softmax(-1).detach().cpu().numpy()[0]
