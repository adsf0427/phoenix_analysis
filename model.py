...
class ResNet(nn.Module):
    def __init__(
        ...
        *,
        ...
        layers += [
            nn.Conv1d(conv_channels, 32, kernel_size=kernel_size, padding="same"),
            actv_builder(),
            nn.Flatten(),
...
class Brain(nn.Module):
    def __init__(
        self, *, conv_channels, num_blocks, kernel_size=3, is_oracle=False, version=1
    ):
        super().__init__()
...
class RankingLoss(nn.Module):
    ...
    def forward(self, outputs, targets):
        _, predicted_ranks = torch.sort(outputs, dim=1, descending=True)
        _, true_ranks = torch.sort(targets, dim=1, descending=True)

        loss = F.mse_loss(predicted_ranks.float(), true_ranks.float())
        return loss


class GRP_V3(nn.Module):
    def __init__(self, hidden_size=64, num_layers=2):
        super().__init__()
        # self.rnn = nn.GRU(input_size=GRP_SIZE, hidden_size=hidden_size, num_layers=num_layers, batch_first=True)
        self.fc = nn.Sequential(
            nn.Linear(GRP_SIZE, hidden_size),
            nn.ReLU(inplace=True),
